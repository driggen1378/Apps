...
export default function HomeScreen() {
  ...
  function handleWriteNewsletter(item) {
    dispatch({
      type: 'SET_NEWSLETTER_SEED',
      seed: { topic: item.pipeline.q2, pipeline: item.pipeline },
    })
  }
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 bg-[#0f1117] text-slate-200">
      ...
      {/* Ready to draft — launch newsletter loop */}
      {readyIdeas.length > 0 && (
          ...
          <ul className="divide-y divide-[#1e3a5f]">
            {readyIdeas.map(item => (
              <li key={item.id} className="flex items-center gap-3 px-4 py-3 hover:bg-[#0d1829] transition-colors">
                ...
                <button
                  onClick={() => handleWriteNewsletter(item)}
                  className="text-xs font-medium text-[#c5a028] hover:text-[#d9b030] border border-[#c5a028]/40 hover:border-[#c5a028] px-3 py-1.5 rounded-lg transition-all shrink-0"
                >
                  Write newsletter →
                </button>
              </li>
