...
export default function HomeScreen() {
  ...
  async function handleSubmit() {
    ...
    try {
      const result = await assessInput(input, state.brand);
  ...
  function handleDiscovery() {
    dispatch({ type: 'SET_ENTRY_MODE', value: 'discovery' });
    dispatch({ type: 'SET_SCREEN', screen: SCREENS.DISCOVERY });
  }
    ...
    <div className="flex flex-col h-screen bg-[#0f1117] text-slate-200">
      ...
      <div className="flex-1 flex flex-col px-8 pb-8 gap-4 min-h-0">
        ...
        <div className="flex items-center gap-3 justify-between">
          ...
          {/* Submit */}
          <div className="flex items-center gap-3">
            <p className="text-xs text-slate-600">Cmd+Enter</p>
            <button
            ...
            >
              {state.isLoading ? <LoadingInline text="Reading…" /> : 'Start drafting →'}
            </button>
          </div>
        </div>
      </div>
