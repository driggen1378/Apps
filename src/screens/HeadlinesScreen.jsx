...
export default function HeadlinesScreen() {
  ...
  // Fire headline search on first entry if no headlines yet
  useEffect(() => {
    if (state.headlines.length === 0 && currentDraft?.draft) {
      fetchHeadlines();
    }
  ...
  async function fetchHeadlines() {
    ...
    try {
      const result = await findHeadlines(currentDraft.draft);
  ...
  function handleUseSubjectLine(plain) {
    dispatch({ type: 'SET_SUBJECT_LINE', value: plain });
  }
    ...
    <div className="flex flex-col h-full min-h-0 px-8 py-6 max-w-3xl mx-auto w-full">
      ...
      <div className="mb-6">
        ...
        <input
          ...
          placeholder="Click 'Use as subject line' below, or type your own"
      ...
      {/* Empty state after a successful but empty search */}
      {!localLoading && !localError && state.headlines.length === 0 && (
        <div className="flex flex-col items-center justify-center gap-3 flex-1 text-center">
          <p className="text-sm text-slate-500">No results returned. Try refreshing.</p>
          <button
            onClick={fetchHeadlines}
            className="text-sm text-slate-300 hover:text-white border border-[#2a2d3e] hover:border-slate-500 px-4 py-2 rounded-lg transition-all"
          >
      ...
      {!localLoading && state.headlines.length > 0 && (
        <div className="flex flex-col gap-3 overflow-y-auto">
          {state.headlines.map((h, i) => (
            <div
              key={i}
              className="bg-[#141620] border border-[#2a2d3e] rounded-xl px-5 py-4 hover:border-slate-500 transition-colors"
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => handleUseSubjectLine(h.plain_version)}
                ...
                >
                  {state.subjectLine === h.plain_version ? '✓ Selected' : 'Use as subject line'}
                </button>
              </div>
            </div>
          ))}
