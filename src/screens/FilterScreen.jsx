...
export default function FilterScreen() {
  ...
  // Auto-run filter check on entry if no results yet
  useEffect(() => {
    if (!results && currentDraft?.draft) {
      runCheck();
    }
  ...
  async function runCheck() {
    ...
    try {
      const result = await runFilterCheck(currentDraft.draft);
      const bothPass = result.brandFilter.pass && result.promiseFilter.pass;
      dispatch({
        type: 'SET_FILTER_RESULTS',
        results: result,
        status: bothPass ? 'passed' : 'failed',
      });
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 px-8 py-6 max-w-3xl mx-auto w-full">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          onClick={() => dispatch({ type: 'SET_SCREEN', screen: SCREENS.DRAFT })}
      ...
      {results && !localLoading && (
        <>
          <div className="grid grid-cols-2 gap-4 mb-6">
            <FilterCard
              title="Brand Filter"
              passed={results.brandFilter.pass}
              explanation={results.brandFilter.explanation}
            />
            <FilterCard
              title="Core Promise"
              passed={results.promiseFilter.pass}
              explanation={results.promiseFilter.explanation}
            />
          </div>
          ...
          {bothPass ? (
            <div className="flex flex-col items-center gap-4 mt-4">
              <div className="text-center">
                <p className="text-sm text-green-400 font-medium mb-1">Both filters passed</p>
                <p className="text-xs text-slate-500">Ready to copy and send.</p>
              </div>
              <button
                onClick={handlePublish}
                className="px-8 py-3 bg-white text-[#0f1117] text-sm font-bold rounded-xl hover:bg-slate-100 transition-all"
              >
                {copied ? '✓ Copied to clipboard' : 'Ready to publish — copy to clipboard'}
              </button>
              {state.subjectLine && (
                <p className="text-xs text-slate-600">
                  Subject: {state.subjectLine}
                </p>
              )}
            </div>
          ) : (
    ...
    >
      ...
      <p
        className={`text-xs leading-relaxed ${
          passed ? 'text-slate-400' : 'text-red-300'
        }`}
      >
        {explanation}
      </p>
  ...
  return (
    <div className="flex gap-2">
      {[0, 1, 2].map((i) => (
        <div
          key={i}
          className="w-3 h-3 rounded-full border-2 border-slate-600 border-t-white animate-spin"
